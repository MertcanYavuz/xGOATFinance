...
contract Vesting is Ownable {
    ...
    constructor(address _tokenAddress, address _usdtTokenAddress, uint256 _icoStartTime, uint256 _icoEndTime) Ownable(msg.sender) {
    ...
    function buyToken(uint256 _usdtAmount) external {
        ...
        require(_checkAllowanceUSDT(_usdtAmount), "Insufficient allowance for transfer");
        uint256 tokenAmount = _calculateTokenAmountFromUSDT(_usdtAmount);
        require(tokenSold + tokenAmount <= maxSaleLimit, "Maximum sale limit reached");
        
        bool transferSuccess = usdtTokenAddress.transferFrom(msg.sender, address(this), _usdtAmount);   // Transfer USDT from address to this contract
    ...
    function claimVestedTokens() external {
        ...
        require(block.timestamp > vestingStartTime && vestingStartTime > 0, "Vesting hasn't started yet or start time is invalid");
        uint256 totalAmount = beneficiaryMap[msg.sender].tokenBalance;
        uint256 claimedAmount = beneficiaryMap[msg.sender].claimedBalance;
        uint256 claimableAmount = _calculateClaimableAmount(msg.sender, totalAmount, claimedAmount);
    ...
    function _checkAllowanceUSDT(uint256 _usdtAmount) internal view returns (bool) {
        return _usdtAmount <= usdtTokenAddress.allowance(msg.sender, address(this));   // Allowance check for USDT amount to spend by this contract 
    }
    
    function _calculateTokenAmountFromUSDT(uint256 _usdtAmount) internal view returns (uint256){
        ...
        uint256 tokenAmount = _usdtAmount * _tokenPriceRate;
    ...
    function _checkBuyLimit(uint256 _usdtAmount) internal view returns (bool) {
        ...
        return _minPurchase <= totalBuyingAmount && totalBuyingAmount <= _maxPurchase;
    }  

    function _calculateClaimableAmount(address _address, uint _totalAmount, uint _claimedAmount) internal view returns (uint) {
        ...
        uint256 installmentAmount = _calculateInstallmentAmount(_totalAmount, TGEAmount, installments); 
    ...
    function _calculateInstallmentAmount(uint256 _total, uint256 _TGEAmount, uint256 _installment) internal pure returns (uint) {
        return ( _total - _TGEAmount ) / _installment;
    }

    function _getCurrentPeriod(uint256 _cliff, uint256 _period, uint256 _installments) internal view returns (uint) {
    ...
    function _checkCliffPassed(uint256 _cliffTime) internal view returns (bool) {
    ...
    function getRemainingBalance(address _address) public view returns (uint) {
        return beneficiaryMap[_address].tokenBalance - beneficiaryMap[_address].claimedBalance;
    }
    ...
    function getAvailableBalance(address _address) public view returns (uint) {
        ...
        uint256 claimableAmount = _calculateClaimableAmount(_address, totalAmount, claimedAmount);
    ...
    function getLockedAmount(address _address) public view returns (uint) {
        return getTotalBalance(_address) - getClaimedBalance(_address) - getAvailableBalance(_address);
    }
    ...
    function setVestingSchedule(Roles _role, uint256 _TGE, uint256 _cliff, uint256 _period, uint256 _installments) public onlyOwner  {
        require(uint8(_role) < uint8(Roles.MAX_ROLE), "Role must be in the roles");
    ...
    function setBuyLimits(Roles _role, uint256 _minPurchase, uint256 _maxPurchase, uint256 _tokenPriceRate) public onlyOwner {
        require(uint8(_role) < uint8(Roles.MAX_ROLE), "Role must be in the roles");
